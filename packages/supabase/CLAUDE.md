...
- `card_id` (text, PK) - Unique card identifier (GENERATED ALWAYS as company_id || '__' || week_id)
- `company_id` (text, FK → company.id) - Associated company
- `week_id` (text) - Week identifier
- `headline` (text) - Card headline
- `bullets_json` (jsonb) - Array of bullet points (max 165 chars each)
- `coverage_top` (text) - Top coverage level: 'high', 'medium', or 'low'
...
Validates that all bullet points in a JSON array are ≤ 165 characters.
- Used as a CHECK constraint on `company_weekly_card.bullets_json`
- 165 char limit provides 5-char buffer beyond the 160-char target for LLM variation
...
8. `20251102191630_change_coverage_top_to_lowercase.sql` - Change coverage_top constraint to lowercase values
9. `20251102193914_increase_bullet_length_limit.sql` - Increase bullet length limit from 160 to 165
...
// Note: card_id is auto-generated by database as company_id || '__' || week_id
const cardData: Omit<TablesInsert<'company_weekly_card'>, 'card_id'> = {
  ...
  coverage_top: 'high',
  significance_max: 9,
  // card_id will be auto-generated as 'factory-ai__2025-W44'
};
...
4. **Bullet Points**: The `bullets_json` field has a validation constraint (165 char max per bullet, provides buffer for 160 char target)
5. **Timestamps**: `company_weekly_card` has auto-updating `updated_at` via trigger
6. **Auto-generated card_id**: The `card_id` field is GENERATED ALWAYS as `company_id || '__' || week_id` - do not provide it in INSERT statements
